...
| D28 | **The fenced insert enforces its fence.** It was named "fenced", its comment named three objects, and it inserted whatever it was handed — in the ONE method where the permission check is skipped. Nothing was exposed (every caller builds fixed types, the published spec is the real allow-list) but a name must not promise a guarantee it does not provide. Five objects, checked per record | 2026-09-20 |
...
- **Permissions:**
  ...
  - **No respondent needs permission on these two objects** — signed-in and guest submits both write
    ...
    to the post office to mail a letter"). The fence list grows from 3 objects to 5, and as of
    2026-09-20 that list is **enforced** rather than described: `insertFenced` checks every
    record's type against `FinalSubmitService.FENCE` and throws on anything else (D28). The one thing a
...
| Whether a site guest user can queue asynchronous Apex                                   | Expected to work; a short proof in the org before any runtime code (F2 spec §11). Platform events are not used — background jobs only (D41)                                                                     |
...
| Where a failed mapping is retried from, and who may retry it                            | A Retry button on the submission, plus a Ready for Retry status for bulk retries within the save's remaining background-job capacity; only Failed and Ready for Retry are retried (D35, revised by D42 and D46) |
...
- **Fenced system-mode insert** — server code that may create only a fixed list of app-owned records,
  enforced by checking each record against that list ("system mode" = the permission check is
  skipped, which is the point: a guest owns nothing and can create nothing, yet must be able to
  leave a submission),
