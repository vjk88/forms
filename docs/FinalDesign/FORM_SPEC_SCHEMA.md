...
```jsonc
{
  ...
  "settings": {
    ...
    "draft": {
      // v2 — Save & Resume DEFERRED (DEFERRED.md #2); always enabled:false in v1
      "enabled": false,
  ...
  "layout": {
    ...
    "options": {
      // THIS primitive's presentation options (catalog §2) — shape varies by type
      "placement": "top",
      "mode": "numbered",
      "navigation": "gated",
      "showStepCount": true
    ...
    "zonesDefault": {
      // layoutZones defaults; pages may override
  ...
  "theme": {
    ...
    "overrides": {
      // SPARSE theme-property deltas (ARCH §4.3) — same shape as a theme
  ...
  "resolved": {
    // PUBLISHED SNAPSHOT ONLY — never present in drafts
    "tokens": {
      "--c-accent": "#0d9488",
      "--c-radius": "14px" /* … full contract */
    },
  ...
  "sections": [
    {
      ...
      "surface": {
        // explicit values — WIN over the preset (the cascade, ARCH §4.3)
        "bg": null,
        "border": null,
        "shadow": null,
        "padding": "md"
      },
      "columns": 2, // 1|2|3|4 (4 added 2026-07-10 — the FormStudio port, BUILDER_SURFACES §3)
      "showHeader": true, // false hides title/icon/description at render (BUILDER_SURFACES §3)
      "collapsible": false,
      ...
      "block": false, // authoring marker: a STANDALONE content block between sections is a
      // wrapper section carrying one content element (the legacy formStudio
      // serialization invariant, CANVAS_RULES §3/§6). block:true sections hold
      // nothing droppable, reorder like sections, render chromeless (style
      // "plain", no title). Runtime treats them as ordinary sections.
...
| `image` / `richText` / `divider` / `spacer` | content blocks                                                                 | src + versionId + alt + size / html / — / size                   |
| `callout` / `consent` / `emptySpace`        | content blocks (BUILDER_SURFACES §1 — the FormStudio roster, added 2026-07-10) | variant + html / html (required = validation sugar) / —          |

Content types (`image`…`emptySpace`) have `binding: null` always. Unknown `type` at runtime renders
a placeholder ("unsupported element"), never crashes the form — same ignore-unknown discipline as §2.

**`field` config additions (BUILDER_SURFACES §2):** `renderAs` (Default | Dropdown | Radio_Buttons |
Checkbox_Group | Custom_MultiSelect | Toggle | Slider), `options` (custom `{label, value}` rows —
custom choices for text fields, or overriding describe options), `slider` (`{min, max, step}`).
`width` (§4 root) = the element's column SPAN inside its section grid; `divider`/`spacer` always
span every column. Validation entries may carry a `preset` key (`email`/`phone`/`url`/`length` +
`minLen`/`maxLen`) — pure editor round-trip sugar; the runtime only ever reads the §7 shape.
...
- **Rules scope per entry:** visibility/validation referencing elements inside a repeatable section
  evaluate against the _same entry's_ values — entry 2's "show if X = Yes" reads entry 2's X.
...
```jsonc
{
  ...
  "rules": [{ "source": "el_m4t8s6vb", "operator": "equals", "value": "Yes" }]
}
```
...
```jsonc
{
  ...
  "repeats": {
    // repeatable sections (§4.1) — array of entry maps per section id
    "sec_p8d2w7rt": [
