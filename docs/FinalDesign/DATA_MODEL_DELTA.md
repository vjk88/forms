...
> hits the cap — just don't _preclude_ it.
...
> _Salesforce itself_ stamped stale old-world values onto every final-created record (a splitHero
...
> value set + defaults get reshaped then. Spec: [HOSTING_ADAPTERS_SPEC.md](./specs/HOSTING_ADAPTERS_SPEC.md),
...
| Change                                  | Why                                                                                                                                                                                                                                                     |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Add `Config_JSON__c`** (LongTextArea) | Section presentation without dedicated fields: style preset + explicit surface values (spec §4 `surface`) and repeat presentation (style / add-remove labels / entry label template). The repeat _binding_ stays in the existing dedicated fields above |
