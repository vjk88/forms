...
- Element-level `analytics` block: `{ role: score|sentimentText|category|none, topics:
[{id, name}], scaleMin, scaleMax }` — scale bounds auto-locked for preset-scale types.
  **Topics are TAGS (owner ruling 2026-07-27, round 6):** picked from the managed
  `Survey_Topic__c` vocabulary (multi-select chips + inline "new topic"), never free-typed;
  a question can carry several.
...
- **L3 — Question Bank (SKIPPED, owner 2026-07-27 round 6):** owner chose **managed topic
  tags** instead — the lighter fix for the same drift problem. Topic comparability now comes
  from the shared `Survey_Topic__c` vocabulary (see §6 topics model); question-wording
  comparability (the bank's other benefit) is consciously dropped. `ranking`-style key
  reservation logic applies: nothing blocks a future bank; the `Form_Element__c` lookup stays
  parked.
...
### Addendum rulings (owner, 2026-07-27, round 6 — topics are tags; bank skipped)

- **Question Bank SKIPPED** (S7 removed from §9). Owner: "skip question bank, but get
  topics."
- **Topics = managed multi-tags.** The model:
  - **`Survey_Topic__c`** — the vocabulary (Name + `Active__c`). Managed by builders:
    the question-properties picker offers the list + inline "new topic";
    `Form_Builder_Admin` gets CRUD. Deactivate hides from the picker, never deletes
    history.
  - **Question ↔ topics = many-to-many**: element `analytics.topics` array of
    `{id, name}` in the spec (name snapshotted at publish for display only).
  - **Answer ↔ topics**: one **`Form_Answer_Topic__c`** junction row per (answer ×
    topic) — lookup to the answer + lookup to the live topic. This is what makes
    "average score by topic" a real report with multi-tag questions.
  - **Rename semantics — tags are LIVE, not snapshots**: renaming a topic record
    updates every chart consistently (that is the point of managed tags). The
    photocopy rule stays for question WORDING only. `Topic_Snapshot__c` on the answer
    becomes a display-only joined string ("Support; Pricing") for eyeballing rows.
  - **Slicing:** storage (both objects + junction writer) lands in **S1**
    (hand-authored specs test it); the chips picker + inline-create + manage UI land
    in **S2** with the question inspectors.
