...
describe('c-final-form-studio', () => {
    ...
    it('Build mode: a polymorphic lookup is minted flagged, with no single target', async () => {
        jest.useFakeTimers();
        loadStudio.mockResolvedValue({
            name: 'Contact us',
            specJson: JSON.stringify(SPEC),
        ...
        const saved = JSON.parse(saveDraft.mock.calls[0][0].specJson);
        const added = saved.pages[0].sections[0].elements.at(-1);
        expect(added.binding.field).toBe('WhatId');
        expect(added.config.inputType).toBe('reference');
        expect(added.config.polymorphic).toBe(true);
        expect(added.config.referenceTo).toBeUndefined();
        jest.useRealTimers();
    });
