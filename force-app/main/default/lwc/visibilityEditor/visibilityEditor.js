...
// Operators offered per field-type family.
const TEXT_OPS = [
...
// Known value sets for $Form.* settings.
...
export default class VisibilityEditor extends LightningElement {
    ...
    fieldOptionsFor(source) {
        switch (source) {
            ...
            default: {
                ...
                // One-level relationship fields (e.g. "Account › Type").
                const traversal = [];
    ...
    // Resolve a "Relationship.Field" token to its lookup field, target object
    // and field type, from the lookup-target metadata.
    ...
    fieldTypeForCond(c) {
        if (c.source === 'user') {
            const api = (c.field || '').replace('$User.', '');
            const f = (this.userFields || []).find((x) => x.apiName === api);
            return f ? f.type : '';
        }
        if (c.source === 'record') {
        ...
        // Profile / Form settings — treat as text.
        return 'STRING';
    }
    ...
    // Decides how the Value control renders for a condition.
    valueKindFor(c, type) {
        const t = (type || '').toUpperCase();
        if (c.source === 'form') {
    ...
    handleSourceChange(e) {
        ...
        // Switching source clears the field (its options changed).
    ...
    handleRowChange(e) {
        ...
        this.conditions = this.conditions.map((c) => {
            ...
            // Changing the field can invalidate the current operator.
            if (prop === 'field') {
    ...
    // Validates the custom logic string: allowed tokens, condition-number
    // range, balanced parentheses and overall syntax. Sets customLogicError.
    validateCustomLogic() {
        this.customLogicError = '';
        if (!this.isCustom) return true;

        // Numbering references row positions, so all rows must be complete.
    ...
    buildJson() {
        const rules = this.conditions
            ...
            .map((c) => {
                ...
                // Traversal rules carry the lookup field + target object so the
                // runtime can fetch the related record's value.
    ...
    // Native required-field validation across every condition row.
    validateInputs() {
        let ok = true;
        this.template
    ...
    handleSave() {
        // Validate both the per-row fields/values and (if used) the custom logic.
        const inputsOk = this.validateInputs();
        const customOk = this.isCustom ? this.validateCustomLogic() : true;
        if (!inputsOk || !customOk) {
            return; // keep the modal open with the errors shown
        }
        this.dispatchEvent(
