...
import uploadImage from '@salesforce/apex/Z_FormAssetController.uploadImage';
import deleteImage from '@salesforce/apex/Z_FormAssetController.deleteImage';
import { RADIUS_OPTIONS, SECTION_STYLE_OPTIONS, LAYOUT_TEMPLATES, FONT_OPTIONS } from 'c/zFormThemes';
...
export default class ZPropertyPanel extends LightningElement {
    @api formId;
    ...
    get themeCustomCss() {
        return this._selection?.theme?.customCss || '';
    }

    handleAiPromptChange(event) {
        this.aiPrompt = event.detail.value;
    }

    handleApplyAiStyle() {
        if (!this.aiPrompt) return;
        this.isAiProcessing = true;
        this.dispatchEvent(new CustomEvent('aistyle', {
            detail: { prompt: this.aiPrompt }
        }));
    }
    ...
    get isSplitLayout() {
        const layout = this._selection?.theme?.layout;
        return layout === 'split' || layout === 'splitSidebarImg' || layout === 'splitBgOverlay' || layout === 'asymmetricLanding' || layout === 'floatingCardSplit';
    }

    get themeSplitRatio() {
        const val = this._selection?.theme?.splitLeftRatio;
        if (!val) {
            const layout = this._selection?.theme?.layout;
            return layout === 'floatingCardSplit' ? 50 : 30;
        }
        return parseInt(val, 10) || 30;
    }

    get themePageBgImage() {
        return this._selection?.theme?.pageBgImage || '';
    }

    get themeCardBgImage() {
        return this._selection?.theme?.cardBgImage || '';
    }

    get themeHeaderBgImage() {
        return this._selection?.theme?.headerBgImage || '';
    }

    handleSplitRatioChange(event) {
        const value = parseInt(event.detail.value, 10) || 30;
        const left = value + '%';
