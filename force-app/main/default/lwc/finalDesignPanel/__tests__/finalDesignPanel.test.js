...
jest.mock(
  ...
  () => ({
    default: jest.fn(() => Promise.resolve('{"palette":{"accent":"#7c2d9c"}}'))
  }),
  { virtual: true }
);
...
async function openGallery(el, which) {
  el.shadowRoot.querySelector(`.entry-change[data-gallery="${which}"]`).click();
  await flush();
...
describe("c-final-design-panel", () => {
  ...
  it("accent change writes a sparse override and emits specchange", async () => {
    ...
    await flush();
    expect(lastSpec(handler).theme.overrides.palette.accent).toBe("#123456");
  });
  ...
  it("theme row: name + Change… opens the gallery popup in theme mode", async () => {
    const el = mount();
    expect(el.shadowRoot.querySelectorAll(".entry-name")[0].textContent).toBe(
      "Nordic Minimalist"
    );
    const gallery = await openGallery(el, "theme");
  ...
  it("theme switch with overrides opens the confirm gate; Keep preserves them", async () => {
    ...
    expect(el.shadowRoot.querySelector("c-final-gallery-picker")).toBeNull();
  ...
  it("custom font pick writes overrides.customFont; built-in pick clears it", async () => {
    ...
    const select = el.shadowRoot.querySelector('select[data-key="typography"]');
    expect(
  ...
  it("Edit… dispatches the explicit themeedit action (blast-radius rule); Change… does not", async () => {
    ...
    el.shadowRoot.querySelector('.entry-change[data-gallery="theme"]').click();
    await flush();
    expect(handler).not.toHaveBeenCalled();
  ...
  it("layout row: label + Change… opens the gallery popup in layout mode", async () => {
    ...
    expect(el.shadowRoot.querySelectorAll(".entry-name")[1].textContent).toBe(
      "Split hero · Conversational"
    );
    const gallery = await openGallery(el, "layout");
