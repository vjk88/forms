...
describe('ruleMatches — operators', () => {
    it('equals / notEquals compare loosely as strings (inputs arrive stringly)', () => {
        ...
        expect(
            ruleMatches({ source: 'a', operator: 'equals', value: 5 }, c)
        ).toBe(true);
        expect(
            ruleMatches({ source: 'b', operator: 'equals', value: '5' }, c)
        ).toBe(true);
        expect(
            ruleMatches({ source: 'c', operator: 'notEquals', value: 'No' }, c)
        ).toBe(true);
        expect(
            ruleMatches({ source: 'missing', operator: 'equals', value: '' }, c)
        ).toBe(true);
    });

    it('equals / notEquals on multi-select arrays check membership, not stringified join', () => {
        const c = ctx({ m: ['OptionA', 'OptionB'], one: ['Solo'], none: [] });
        // pre-fix: String(['OptionA','OptionB']) === "OptionA,OptionB" never matched
        expect(
            ruleMatches(
                { source: 'm', operator: 'equals', value: 'OptionA' },
                c
            )
        ).toBe(true);
        expect(
            ruleMatches(
                { source: 'm', operator: 'equals', value: 'OptionC' },
                c
            )
        ).toBe(false);
        expect(
            ruleMatches({ source: 'one', operator: 'equals', value: 'Solo' }, c)
        ).toBe(true);
        // notEquals is the exact complement — the pair always partitions
        expect(
            ruleMatches(
                { source: 'm', operator: 'notEquals', value: 'OptionA' },
                c
            )
        ).toBe(false);
        expect(
            ruleMatches(
                { source: 'm', operator: 'notEquals', value: 'OptionC' },
                c
            )
        ).toBe(true);
        // empty selection equals nothing (blank checks stay isBlank's job)
        expect(
            ruleMatches({ source: 'none', operator: 'equals', value: '' }, c)
        ).toBe(false);
        expect(
            ruleMatches(
                { source: 'none', operator: 'notEquals', value: 'X' },
                c
            )
        ).toBe(true);
        // numeric scale answers stored as numbers still match string rule values
        const n = ctx({ picks: [1, 3] });
        expect(
            ruleMatches({ source: 'picks', operator: 'equals', value: '3' }, n)
        ).toBe(true);
    });
    ...
    it('contains: case-insensitive substring; arrays check membership', () => {
        ...
        expect(
            ruleMatches(
                { source: 't', operator: 'contains', value: 'world' },
                c
            )
        ).toBe(true);
        expect(
            ruleMatches({ source: 'm', operator: 'contains', value: 'C' }, c)
        ).toBe(true);
        expect(
            ruleMatches({ source: 'm', operator: 'contains', value: 'B' }, c)
        ).toBe(false);
    });
    ...
    it('isBlank / isNotBlank treat "", null, undefined, [] as blank', () => {
        ...
        expect(ruleMatches({ source: 'arr', operator: 'isBlank' }, c)).toBe(
            true
        );
        expect(ruleMatches({ source: 'nope', operator: 'isBlank' }, c)).toBe(
            true
        );
        expect(ruleMatches({ source: 'v', operator: 'isNotBlank' }, c)).toBe(
            true
        );
    });
    ...
    it('greaterThan/lessThan: numeric when both parse; FALSE when not comparable (§7 fail-safe)', () => {
        ...
        expect(
            ruleMatches({ source: 'n', operator: 'greaterThan', value: 40 }, c)
        ).toBe(true);
        expect(
            ruleMatches({ source: 'n', operator: 'lessThan', value: 40 }, c)
        ).toBe(false);
        // "abc" > "abb" would be true as strings — must be FALSE, never silent
        expect(
            ruleMatches(
                { source: 's', operator: 'greaterThan', value: 'abb' },
                c
            )
        ).toBe(false);
    });
    ...
    it('date sources compare as dates', () => {
        ...
        expect(
            ruleMatches(
                { source: 'd', operator: 'greaterThan', value: '2026-01-01' },
                c
            )
        ).toBe(true);
        expect(
            ruleMatches(
                { source: 'd', operator: 'lessThan', value: '2026-01-01' },
                c
            )
        ).toBe(false);
    });
...
describe('evaluateVisibility — logic + action', () => {
    ...
    it('no config / empty rules → visible', () => {
        ...
        expect(evaluateVisibility({ action: 'show', rules: [] }, ctx({}))).toBe(
            true
        );
    });
    ...
    it('all (AND) vs any (OR)', () => {
        const c = ctx({ a: 'Yes', b: '' });
        expect(
            evaluateVisibility({ action: 'show', logic: 'all', rules }, c)
        ).toBe(false);
        expect(
            evaluateVisibility({ action: 'show', logic: 'any', rules }, c)
        ).toBe(true);
    });
    ...
    it('hide inverts the match', () => {
        ...
        expect(
            evaluateVisibility({ action: 'hide', logic: 'all', rules }, c)
        ).toBe(false);
        expect(
            evaluateVisibility(
...
describe('evaluateCustomLogic parser', () => {
    it('parses precedence and parens; rejects garbage and bad indexes', () => {
        expect(evaluateCustomLogic('1 OR 2 AND 3', [true, false, false])).toBe(
            true
        ); // AND binds tighter
        expect(
            evaluateCustomLogic('(1 OR 2) AND 3', [true, false, false])
        ).toBe(false);
...
describe('validation entries (§7)', () => {
    ...
    it('pattern skips blanks; malformed regex never blocks', () => {
        ...
        expect(
            validateEntry(
                { type: 'pattern', pattern: '(', message: 'x' },
                'abc',
                ctx({})
            )
        ).toBe(null);
    ...
    it('custom compareTo: Email == ConfirmEmail', () => {
        ...
        expect(validateEntry(e, 'a@x.com', ctx({ confirm: 'a@x.com' }))).toBe(
            null
        );
        expect(validateEntry(e, 'a@x.com', ctx({ confirm: 'b@x.com' }))).toBe(
            'Must match'
        );
    });
    ...
    it('when gates: the entry only applies when its condition holds', () => {
        const e = {
            ...
            when: {
                logic: 'all',
                rules: [{ source: 'other', operator: 'equals', value: 'Yes' }]
            }
        };
